...
// This allows this handler act like a middleware
// Every request came here first and if pass the next handler is called
...
func loginHandler(w http.ResponseWriter, r *http.Request) {
	...
	case "login":
		...
		if err != nil {
			http.Error(w, fmt.Sprintf("Error when thying to get provider %s: %s", provider, err), http.StatusBadRequest)
			return
		}
